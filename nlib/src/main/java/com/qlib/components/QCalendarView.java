 ...
 * 
 */
public class QCalendarView extends View implements View.OnTouchListener {
	private final static String TAG = "anCalendar";
	private Date selectedStartDate; // 选择的开始日期
	...
	private int CellDataA[] = {}; // 重绘数据, 正常
	private int CellDataB[] = {}; // 重绘数据, 异常 迟到早退，位置不符
	private int CellDataC[] = {}; // 重绘数据,  旷工

	private Drawable cellbga;
	private Drawable cellbgb;
	private Drawable cellbgc;

	public QCalendarView(Context context) {
		super(context);
		init();
	}

	public QCalendarView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < surface.weekText.length; i++) {
			// 画星期背景 beign
			Paint npt = surface.cellBgPaint;
			npt.setColor(Color.parseColor("#f1f1f1"));
			float left = i * surface.cellWidth;
			float top = surface.monthHeight + surface.weekHeight + (-1)
			...
			// - surface.borderWidth, npt); // 暂时取消 画星期背景
		...
		//drawNormalCellBg(canvas, CellDataA, cellbga);
		//drawUnNormalCellBg(canvas, CellDataB, cellbgb);
		//drawPositionCellBg(canvas, CellDataC, cellbgc);
		//drawUnNormalCellBg(canvas, CellDataC, cellbgc); // 位置不符也画在迟到早退中，统一成异常
		//drawNormalCellBg(canvas, CellDataD, cellbgd);
		
		drawCellBg(canvas, CellDataA, cellbga);
		drawCellBg(canvas, CellDataB, cellbgb);
		drawCellBg(canvas, CellDataC, cellbgc);

		super.onDraw(canvas);
	}

	// 画考勤异常标签
	private void drawCellBg(Canvas canvas, int CellData[],
			Drawable drawable) {
		if (CellData == null) {
			return;
		}
		...
		for (int j = 0; j < CellData.length; j++) {
			for (int i = 0; i < date.length; i++) {
				...
				if (date[i] == CellData[j]) {
					int x = getXByIndex(i);
					int y = getYByIndex(i);
					float left = surface.cellWidth * (x - 1)
							+ surface.borderWidth + surface.cellWidth / 2;
					float top = surface.monthHeight + surface.weekHeight
							+ (y - 1) * surface.cellHeight
							+ surface.borderWidth;

					Bitmap bitmap = drawableToBitmap(drawable);
					canvas.drawBitmap(bitmap, left + 10, top + 1,
							surface.cellBgPaint);
				}
			}
		}
	...
	// 画位置不符标签
	private void drawPositionCellBg(Canvas canvas, int CellData[],
			Drawable drawable) {
		if (CellData == null) {
			return;
		}
			...
			for (int i = 0; i < date.length; i++) {
				...
				if (date[i] == CellData[j]) {
					...
					float left = surface.cellWidth * (x - 1)
							+ surface.borderWidth + surface.cellWidth / 2;
					float top = surface.monthHeight + surface.weekHeight
							+ (y - 1) * surface.cellHeight + surface.cellHeight
							/ 2 + surface.borderWidth;
	...
	// 画考勤正常标签
	private void drawNormalCellBg(Canvas canvas, int CellData[],
			Drawable drawable) {
		if (CellData == null) {
			return;
		}
		...
		for (int j = 0; j < CellData.length; j++) {
			for (int i = 0; i < date.length; i++) {
				...
				if (date[i] == CellData[j]) {
					...
					Bitmap bitmap = drawableToBitmap(drawable);
					canvas.drawBitmap(bitmap, left + 16, top + 1,
							surface.cellBgPaint); // getResources().getDrawable(R.drawable.signon_bg)
				}
			}
		}
	}

	public void setData(int CellDataA[], Drawable cellbga, int CellDataB[],
			Drawable cellbgb, int CellDataC[], Drawable cellbgc) {
		this.CellDataA = CellDataA;
		this.cellbga = cellbga;

		this.CellDataB = CellDataB;
		this.cellbgb = cellbgb;

		this.CellDataC = CellDataC;
		this.cellbgc = cellbgc;
	...
	private void drawDownOrSelectedBg(Canvas canvas) {
		...
		if (!selectedEndDate.before(showFirstDate)
			...
			for (int i = section[0]; i <= section[1]; i++) {
				drawCellBg(canvas, i, surface.cellSelectedColor); // 画背景色
			}
		}
	}
		...
		case MotionEvent.ACTION_UP: {
			if (downDate != null) {
				if (isSelectMore) { // 多选
					...
					String tpday = format.format(selectedStartDate);
					int iday = Integer.valueOf(tpday);

					// Toast.makeText(getContext(), String.valueOf(downIndex) +
					// "," + ",", Toast.LENGTH_SHORT).show();
					Log.i("downIndex", String.valueOf(downIndex));
					Log.i("curStartIndex", String.valueOf(curStartIndex));
					Log.i("curEndIndex", String.valueOf(curEndIndex));

					if (downIndex >= curStartIndex && downIndex < curEndIndex) {
						onItemClickListener.OnItemClick(selectedStartDate,
								selectedEndDate, downDate);
						/*
						 * if (CellDataA != null) { for (int i = 0; i <
						 * CellDataA.length; i++) { if (iday == CellDataA[i]) {
						 * // 响应监听事件 onItemClickListener.OnItemClick(
						 * selectedStartDate, selectedEndDate, downDate); } } }
						 * 
						 * if (CellDataB != null) { for (int i = 0; i <
						 * CellDataB.length; i++) { if (iday == CellDataB[i]) {
						 * // 响应监听事件 onItemClickListener.OnItemClick(
						 * selectedStartDate, selectedEndDate, downDate); } } }
						 * 
						 * if (CellDataC != null) { for (int i = 0; i <
						 * CellDataC.length; i++) { if (iday == CellDataC[i]) {
						 * // 响应监听事件 onItemClickListener.OnItemClick(
						 * selectedStartDate, selectedEndDate, downDate); } } }
						 */
					}
	...
	private class Surface {
		...
		public int bgColor = Color.parseColor("#f7f7f7");
		private int weekbgColor = Color.parseColor("#f1f1f1");
		...
		public int cellDownColor = Color.parseColor("#dddddd");
		public int cellSelectedColor = Color.parseColor("#dddddd"); // 99CCFF
		...
		public void init() {
			...
			weekPaint.setTextSize(weekTextSize);// weekTextSize); // 星期画笔字大小
