...
public class HVListView2 extends ListView {
	...
	/** 构造函数 */
	public HVListView2(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		if (screenWidth == 0) {
			...
			if (getChildAt(0) != null) {
				screenWidth -= (((ViewGroup) getChildAt(0)).getChildAt(0)
						.getMeasuredWidth() + ((ViewGroup) getChildAt(0))
						.getChildAt(1).getMeasuredWidth());
			} else if (mListHead != null) {
				// 减去固定第一列和第二列
				screenWidth -= (mListHead.getChildAt(0).getMeasuredWidth() + mListHead
						.getChildAt(1).getMeasuredWidth());
			}
		}
		return screenWidth;
