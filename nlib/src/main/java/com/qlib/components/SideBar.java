...
public class SideBar extends View {
	...
	// 26个字母
	public static String[] mLetter = { "#", "A", "B", "C", "D", "E", "F", "G",
	...
	protected void onDraw(Canvas canvas) {
		...
		int singleHeight = height / mLetter.length;// 获取每一个字母的高度

		for (int i = 0; i < mLetter.length; i++) {
			paint.setColor(Color.rgb(33, 65, 98));
			paint.setTypeface(Typeface.DEFAULT_BOLD);
			paint.setAntiAlias(true);
			float cellTextSize = singleHeight * 0.8f;
			paint.setTextSize(cellTextSize);// 字体大小
			// 选中的状态
			if (i == choose) {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		final int c = (int) (y / getHeight() * mLetter.length);// 点击y坐标所占总高度的比例*b数组的长度就等于点击b中的个数.
